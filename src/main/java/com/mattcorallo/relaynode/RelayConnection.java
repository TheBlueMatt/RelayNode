...
public abstract class RelayConnection implements StreamParser {
	...
	private static final long MAGIC_BYTES = 0x42BEEF42;
	...
	public synchronized void sendBlock(Block b) {
		try {
			...
			relayPeer.writeBytes(ByteBuffer.allocate(24)
					.putLong(MAGIC_BYTES).putLong(MessageTypes.BLOCK.ordinal()).putLong(blockHeader.length)
					.array());
			...
			relayPeer.writeBytes(ByteBuffer.allocate(24)
					.putLong(MAGIC_BYTES).putLong(MessageTypes.END_BLOCK.ordinal()).putLong(0)
					.array());
		} catch (IOException e) {
	...
	private synchronized void sendTransaction(Transaction t, boolean cacheSend) {
		try {
			...
			relayPeer.writeBytes(ByteBuffer.allocate(24)
					.putLong(MAGIC_BYTES).putLong(MessageTypes.TRANSACTION.ordinal()).putLong(transactionBytes.length)
					.array());
	...
	public int receiveBytes(ByteBuffer buff) throws Exception {
		...
		try {
			...
			LogLine("Corrupted data read from relay peer");
