...
bool RelayConnectionProcessor::process_version_message(size_t read_pos) {
...
bool RelayConnectionProcessor::process_max_version_message(size_t read_pos) {
	char data[ntohl(current_msg.length) + 1];
	memcpy(data, &read_buff[read_pos], ntohl(current_msg.length));
...
ssize_t RelayConnectionProcessor::process_block_message(size_t read_pos) {
...
bool RelayConnectionProcessor::process_transaction_message(size_t read_pos, bool outOfBand) {
...
bool RelayConnectionProcessor::process_ping_message(size_t read_pos) {
...
void RelayConnectionProcessor::recv_bytes(char* buf, size_t len) {
	//XXX: Fix the triple copy shit we've got going here...
