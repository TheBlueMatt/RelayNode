...
protected:
	FlaggedArraySet send_tx_cache;
private:
	FlaggedArraySet recv_tx_cache;
...
public:
	...
	bool maybe_recv_tx_of_size(uint32_t tx_size, bool debug_print) {
		if (tx_size > MAX_RELAY_TRANSACTION_BYTES && (recv_tx_cache.flagCount() >= MAX_EXTRA_OVERSIZE_TRANSACTIONS || tx_size > MAX_RELAY_OVERSIZE_TRANSACTION_BYTES)) {
			if (debug_print)
	...
	void recv_tx(std::shared_ptr<std::vector<unsigned char > > tx) {
		...
		assert(maybe_recv_tx_of_size(tx_size, true));
